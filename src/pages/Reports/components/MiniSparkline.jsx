// Composant micro-sparkline vectoriel SVG léger pour les cartes KPI du module Reporting & Insights
  ...
  return (
    <div className="inline-flex items-center shrink-0" style={{ width, height }}>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
        <defs>
          <linearGradient id={`grad-rep-${color.replace('#', '')}`} x1="0" y1="0" x2="0" y2="1">
            <stop offset="0%" stopColor={color} stopOpacity="0.28" />
            <stop offset="100%" stopColor={color} stopOpacity="0.0" />
          </linearGradient>
        </defs>
        <polygon points={fillArea} fill={`url(#grad-rep-${color.replace('#', '')})`} />
        <polyline
          fill="none"
          stroke={color}
          strokeWidth="2"
